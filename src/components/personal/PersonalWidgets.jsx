...
const ListWidget = ({ title, icon, field, placeholder }) => {
  ...
  return <div><div className="section-header"><h2>{icon} {title}</h2></div><div className="gorev-formu-satir"><input placeholder={placeholder} value={value} onChange={e => setValue(e.target.value)} /><button type="button" className="ders-buyuk-buton" onClick={add}>+ Ekle</button></div><div className="kisisel-liste">{(personalData[field] || []).map(item => <label className={`kisisel-liste-satiri ${item.tamamlandi ? 'tamamlandi' : ''}`} key={item.id}><input type="checkbox" checked={item.tamamlandi} onChange={() => setPersonalData(prev => ({ ...prev, [field]: prev[field].map(entry => entry.id === item.id ? { ...entry, tamamlandi: !entry.tamamlandi } : entry) }))} /><span>{item.ad}</span></label>)}</div></div>;
};

export const MoodTrackerWidget = () => {
  const { personalData, setPersonalData } = useApp();
  const moods = [{ ad: 'Harika', renk: '#22c55e' }, { ad: 'İyi', renk: '#84cc16' }, { ad: 'Dengeli', renk: '#eab308' }, { ad: 'Yorgun', renk: '#f97316' }, { ad: 'Zor', renk: '#ef4444' }];
  const colorForDate = date => personalData.modlar?.[date];
  return <div><div className="section-header"><h2>🌤️ Mod Takibi</h2></div><p className="widget-yardimci-metin">Bir gün seç, sonra o günün ruh halini renk ile kaydet.</p><MonthCalendar markedDates={personalData.modlar || {}} colorForDate={colorForDate} onSelect={date => setPersonalData(prev => ({ ...prev, modSeciliTarih: date }))} selectedDate={personalData.modSeciliTarih} /><div className="mod-secim">{moods.map(mood => <button type="button" key={mood.ad} className={colorForDate(personalData.modSeciliTarih || today()) === mood.renk ? 'aktif' : ''} onClick={() => setPersonalData(prev => ({ ...prev, modlar: { ...(prev.modlar || {}), [prev.modSeciliTarih || today()]: mood.renk } }))}>{mood.ad}</button>)}</div></div>;
};
...
export const PersonalHabitWidget = () => {
  ...
  return <div><div className="section-header"><h2>🌱 Alışkanlık Takibi</h2><strong className="seri-rozeti">🔥 {getStreak(activeDates)} gün</strong></div><div className="gorev-formu-satir"><input placeholder="Yeni alışkanlık" value={value} onChange={e => setValue(e.target.value)} /><button type="button" className="ders-buyuk-buton" onClick={add}>+ Ekle</button></div><div className="kisisel-habitler">{habits.map(habit => <button type="button" className={activeHabit?.id === habit.id ? 'aktif' : ''} key={habit.id} onClick={() => setPersonalData(prev => ({ ...prev, seciliAliskanlik: habit.id }))}>{habit.ad}</button>)}</div>{activeHabit && <MonthCalendar markedDates={markedDates} onSelect={date => setPersonalData(prev => ({ ...prev, aliskanlikKayitlari: { ...(prev.aliskanlikKayitlari || {}), [activeHabit.id]: { ...(prev.aliskanlikKayitlari?.[activeHabit.id] || {}), [date]: true } } }))} />}</div>;
};
...
export const PersonalCycleWidget = () => {
  ...
  return <div><div className="section-header"><h2>🌙 Regl Takibi</h2></div><p className="widget-yardimci-metin">Başlangıç ve süreyi seç; tahmini dönemi soluk renkle gör. Bu bir tıbbi tahmin değildir.</p><div className="gorev-formu-satir"><input type="date" value={cycle.baslangic || ''} onChange={e => setPersonalData(prev => ({ ...prev, regl: { ...(prev.regl || {}), baslangic: e.target.value } }))} /><input type="number" min="1" max="14" placeholder="Kaç gün?" value={cycle.sure || ''} onChange={e => setPersonalData(prev => ({ ...prev, regl: { ...(prev.regl || {}), sure: e.target.value } }))} /></div><MonthCalendar markedDates={marked} /></div>;
};
...
export const PersonalSubscriptionWidget = () => {
  ...
  return <div><div className="section-header"><h2>🔁 Abonelikler</h2></div><div className="abonelik-formu"><select value={form.ad} onChange={e => setForm({ ...form, ad: e.target.value, paket: 'Kişisel' })}>{Object.keys(ABONELIKLER).map(name => <option key={name}>{name}</option>)}</select><select value={form.paket} onChange={e => setForm({ ...form, paket: e.target.value })}>{Object.keys(plans).map(plan => <option key={plan}>{plan}</option>)}</select><input type="number" placeholder={`Fiyat (${plans[form.paket] || 0} TL)`} value={form.ozelFiyat} onChange={e => setForm({ ...form, ozelFiyat: e.target.value })} /><input type="date" value={form.yenileme} onChange={e => setForm({ ...form, yenileme: e.target.value })} /><button type="button" className="ders-buyuk-buton" onClick={add}>+ Ekle</button></div><div className="abonelik-listesi">{(personalData.abonelikler || []).map(item => <div className="abonelik-satiri" key={item.id}><strong>{item.ad} · {item.paket}</strong><span>{item.fiyat} TL · {item.yenileme} yenileme</span></div>)}</div></div>;
};
