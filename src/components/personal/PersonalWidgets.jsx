...
const ListWidget = ({ title, icon, field, placeholder }) => {
  ...
  return <div className="kisisel-widget kisisel-widget-liste"><div className="section-header"><h2>{icon} {title}</h2><span className="widget-mini-ozet">{completed}/{items.length}</span></div><div className="gorev-formu-satir"><input placeholder={placeholder} value={value} onChange={e => setValue(e.target.value)} /><button type="button" className="ders-buyuk-buton" onClick={add}>+ Ekle</button></div><div className="kisisel-liste">{items.map(item => <div className={`kisisel-liste-satiri ${item.tamamlandi ? 'tamamlandi' : ''}`} key={item.id}><input type="checkbox" checked={item.tamamlandi} onChange={() => setPersonalData(prev => ({ ...prev, [field]: prev[field].map(entry => entry.id === item.id ? { ...entry, tamamlandi: !entry.tamamlandi } : entry) }))} /><span>{item.ad}</span><button type="button" className="kisisel-sil-btn" onClick={() => remove(item.id)} aria-label={`${item.ad} kaydını sil`}>×</button></div>)}</div></div>;
};
