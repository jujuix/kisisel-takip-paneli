...
export const DersModule = () => {
  const { dersData, setDersData, activeDersTab, simgesi, showConfirm } = useApp();
  ...
  const handleAddRecord = () => {
    ...
    const newRec = {
      ...
      tip: recordType,
      tarih: todayStr,
      soru: Number(recSoru) || 0,
      dogru: Number(recDogru) || 0,
      yanlis: Number(recYanlis) || 0,
      saat: Number(recSaat) || 0,
      not: recNot
    };
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
      {/* 2. KONULAR SEKMESİ */}
      {activeDersTab === 'ders_konular' && (
        <div className="ders-sekme-panel">
              ...
              <div key={ders.id} className="ders-karti acik" style={{ marginBottom: '14px' }}>
                <div className="section-header">
                  ...
                  <button className="kategori-sil-btn" onClick={() => {
                    showConfirm({
                      title: 'Dersi Sil',
                      message: `"${ders.ad}" dersini ve tüm konularını silmek istediğinize emin misiniz?`,
                      confirmText: 'Sil',
                      isDanger: true,
      ...
      {/* 3. DENEMELER SEKMESİ */}
      {activeDersTab === 'ders_denemeler' && (
        <div className="ders-sekme-panel" style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
          <div className="ders-karti">
            <div className="section-header"><h2>{simgesi("📝")} Yeni Deneme Ekle</h2></div>
          ...
          <div className="ders-karti">
            <div className="section-header"><h2>{simgesi("📊")} Deneme Geçmişi</h2></div>
      ...
      {/* 4. YANLIŞLARIM SEKMESİ */}
      {activeDersTab === 'ders_yanlislar' && (
        <div className="ders-sekme-panel" style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
          <div className="ders-karti">
            <div className="section-header"><h2>{simgesi("📊")} Yanlış Sebebi Analizi</h2></div>
          ...
          <div className="ders-karti">
            <div className="section-header"><h2>{simgesi("➕")} Yanlış Soru Ekle</h2></div>
            <div className="yanlis-formu">
              ...
              <input 
                ...
                onChange={e => {
                  ...
                  reader.onload = () => {
                    setWrongMedia(reader.result);
                    setWrongMediaType(file.type.startsWith('video') ? 'video' : 'image');
                  };
          ...
          <div className="ders-karti">
            <div className="section-header"><h2>{simgesi("❌")} Yanlış Arşivim</h2></div>
            <div className="yanlis-grid">
              {(dersData.yanlislar || []).map(y => (
                <div key={y.id} className="yanlis-karti">
                  ...
                  <div className="yanlis-etiket-satiri">
                    <span>{y.ders} · {y.konu}</span>
                    <span>{y.tarih}</span>
                  </div>
                  ...
                  <div className="yanlis-karti-alt">
                    <button 
                      className={`yanlis-tekrar-durum ${y.tekrarEdildi ? 'edildi' : 'edilmedi'}`}
                      onClick={() => setDersData(prev => ({ ...prev, yanlislar: prev.yanlislar.map(x => x.id === y.id ? { ...x, tekrarEdildi: !x.tekrarEdildi } : x) }))}
                    >
      ...
      {/* 5. ÇALIŞMA TAKİBİ */}
      {activeDersTab === 'ders_takip' && (
        <div className="ders-sekme-panel" style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
          <div className="ders-karti">
            <div className="section-header"><h2>{simgesi("🔥")} Çalışma Takvimi</h2></div>
            <div className="heatmap-grid" style={{ marginTop: '14px' }}>
              {Array.from({ length: 30 }).map((_, i) => {
                ...
                return (
                  <div 
                    key={day} 
                    className={`heatmap-gun ${isWorked ? 'dolu' : ''}`}
                    title={dStr}
          ...
          <div className="ders-karti">
            <div className="section-header"><h2>{simgesi("🎯")} Hedefler</h2></div>
            <div className="gorev-formu-satir">
            ...
            <div className="konu-durum-secimi">
              {KONU_DURUMLARI.map(d => (
                <button
                  ...
                  className={topicModal.konu.durum === d.id ? 'aktif' : ''}
                  onClick={() => {
                    setDersData(prev => ({
                      ...prev,
                      dersler: prev.dersler.map(ders => ders.id === topicModal.dersId ? {
                        ...ders,
                        konular: ders.konular.map(k => k.id === topicModal.konu.id ? { ...k, durum: d.id } : k)
                      } : ders)
                    }));
